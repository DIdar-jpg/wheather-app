...
export default function Dropdown() {
  ...
  return (
    <>
        ...
        <span>EN</span>
        <IoIosArrowForward className='group-focus:rotate-90'/>
        <div className="absolute z-10	shadow-lg -bottom-40 left-0 w-full h-max p-2 bg-white border border-zinc-200 rounded-lg flex flex-col gap-2">
            {langs.map( lang => <span className="flex flex-row gap-2 items-center hover:bg-zinc-100 p-2 rounded-lg"><p>{lang}</p></span>)}
        </div>
        </button>
    </>
  )
